...
use super::windows::{
    add_account::AddAccountWindow,
    clone_configuration::{ CloneControlsData, CloneControlsWindow },
        ...
        match window.kind.clone() {
            EWindow::AddAccount => {
                self.current_window = Some((
                    window.kind.clone(),
                    Box::new(AddAccountWindow::new(shared_data, window.clone())),
                ));
            }
            EWindow::CloneSettings(_info) => {
                self.current_window = Some((
                    window.kind.clone(),
                    Box::new(CloneControlsWindow::new(shared_data, window.clone())),
                ));
            }
            EWindow::Settings => {
                self.current_window = Some((
                    window.kind.clone(),
                    Box::new(
                        super::windows::settings::SettingsWindow::new(shared_data, window.clone())
                    ),
                ));
            }
        }
...
pub trait SubWindow {
    fn new(shared_data: WindowSharedData, window_descriptor: WindowDescriptor) -> Self
        where Self: Sized;
